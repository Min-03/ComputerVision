...
class PositionalEncoding_drophead(nn.Module):
    """
...
class MultiHeadAttention_drophead(nn.Module):
    """
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # implementation).
        self.key = nn.Linear(embed_dim, embed_dim, dtype=torch.float64)
        self.query = nn.Linear(embed_dim, embed_dim, dtype=torch.float64)
        self.value = nn.Linear(embed_dim, embed_dim, dtype=torch.float64)
        self.proj = nn.Linear(embed_dim, embed_dim, dtype=torch.float64)
    ...
    def forward(self, query, key, value, mode, attn_mask=None):
        """
