...
class CaptioningSolverTransformer_drophead(object):
    """
    ...
    def _step(self):
        ...
        t_features = torch.Tensor(features).cuda()
        t_captions_in = torch.LongTensor(captions_in).cuda()
        t_captions_out = torch.LongTensor(captions_out).cuda()
        t_mask = torch.LongTensor(mask).cuda()
        logits = self.model(t_features.type(torch.float64), t_captions_in, mode="train")
