...
class CaptioningSolverTransformer(object):
    ...
    def _step(self):
        ...
        t_features = torch.Tensor(features).to(device)
        t_captions_in = torch.LongTensor(captions_in).to(device)
        t_captions_out = torch.LongTensor(captions_out).to(device)
        t_mask = torch.LongTensor(mask).to(device)
